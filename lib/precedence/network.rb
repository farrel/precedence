	 ...
	 class Network
 		 ...
 		 def add_activity(activity)
  			 if reference_exists?(activity.reference)
   				 fail "Activity #{activity.reference} already exists in the "\
    					'network.'
   			end

  			 unless (activity.post_activities == [])
   				 fail 'Can not add an activity with post activities.'
   			end

  			 unless (activity.pre_activities == [])
   				 fail 'Can not add an activity with pre activities.'
   			end
 		 ...
 		 def connect(pre_ref, *post_refs)
  			 unless reference_exists?(pre_ref)
   				 fail "Pre-activity with reference #{pre_ref} "\
    					'was not found.'
   			end

  			 post_refs.each do |post_ref|
   				 unless reference_exists?(post_ref)
    					 fail "Post-activity with reference #{post_ref} "\
     						'was not found.'
    				end
   			end
