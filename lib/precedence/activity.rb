...
module Precedence
    ...
    # The maximum duration of the activity
 		 ...
 		 def initialize(reference, &block)
  			 reference.respond_to?(:to_s) ? nil : fail('Parameter reference'\
  			 ...
  			 if (reference != Precedence::StartActivity::REFERENCE) &&
   				(reference != Precedence::FinishActivity::REFERENCE)
   				 ...
   				 fail "Activity reference '#{reference}' is reserved."
   			end
 	  ...
 	  # *Note*: When using the Network class it is better to use
 		 ...
 		 def to_s #:nodoc:
  			 "Reference: #{reference}\n"\
   				...
   				(@pre_activities.map do |activity|
   					 activity.reference
   				end).join(',')
  		end
 		 ...
 		 def eql?(other)
  			 if (reference == other.reference) and
 		 ...
 		 def duration_type=(type) #:nodoc:
  			 case (type)
   				...
   				MINIMUM_DURATION, MAXIMUM_DURATION
   					 ...
   					 fail "Duration type '#{type}' is unknown."
   			end
  		end
 		 ...
 		 def start_type=(type) #:nodoc:
  			 case (type)
   				when EARLIEST_START, LATEST_START, DROP_START
   					 ...
   					 fail "Start type '#{type}' is unknown."
   			end
  		end
 		 ...
 		 def self.from_yaml(yaml)
  			 from_yaml_object(YAML.load(yaml))
  		end
	 ...
	 class StartActivity < Precedence::Activity #:nodoc:
 		 ...
 		 def self.from_yaml_object(yamlObj) #:nodoc:
  			 ...
  			 if (reference == REFERENCE)
   				 ...
   				 fail("A StartActivity can only have a reference of '#{REFERENCE}'."\
 		 ...
 		 def register_pre_activity(_activity) #:nodoc:
  			 fail 'This activity can not be a post-activity of any other '\
   			'activity.'
  		end
 	end
	 ...
	 class FinishActivity < Precedence::Activity #:nodoc:
 		 ...
 		 def self.from_yaml_object(yamlObj) #:nodoc:
  			 ...
  			 if (reference == REFERENCE)
   				 ...
   				 fail "A FinishActivity can only have a reference of '#{REFERENCE}'."\
    				" Given reference was '#{reference}'."
   			end
  		end
 		 ...
 		 def register_post_activity(_activity) #:nodoc:
  			 fail 'This activity can not be a pre-activity of any other '\
   			'activity.'
  		end
 	end
end
